...
export default function PassengerRatingModal({
    rideId, passengerId, passengerName, origin, destination, onClose, onSuccess,
}: Props) {
    ...
    async function submit() {
        ...
        setError("");
        try {
            const res = await apiFetch("/ratings/passenger", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ ride_id: rideId, passenger_id: passengerId, stars, comment: comment || null }),
            });
            if (res.ok) {
                onSuccess();
            } else {
    ...
    return (
        <div className="modal-backdrop" onClick={onClose}>
            <div className="modal-box" onClick={(e) => e.stopPropagation()}>
                ...
                <h2 className="modal-title">Évaluer le passager</h2>
                <p className="modal-subtitle">
                    {origin} → {destination} · <strong>{passengerName}</strong>
                </p>
